local HostileTeams = {
	...
	["Black Shadow Division"] = {
		["Class - D"] = true,
		["Hostile Forces"] = true,
		["Foundation Personnel"] = true,
		["Security Department"] = true,
		["Research Department"] = true,
		["Health Department"] = true,
		["Engineering & Technical Services"] = true,
		["Department of External Affairs"] = true,
		["Ethics Committee"] = true,
		["Mobile Task Forces"] = true,
	},

	["Class - D"] = {
		["Administration Department"] = true,
		["Black Shadow Division"] = true,
		["Department of External Affairs"] = true,
		["Engineering & Technical Services"] = true,
		["Ethics Committee"] = true,
		["Foundation Personnel"] = true,
...
local function areTeamsHostile(attackerTeam, victimTeam)
	if attackerTeam and victimTeam and HostileTeams[attackerTeam] then
		return HostileTeams[attackerTeam][victimTeam] == true
	end
	return false
end
...
function CalculateDMG(SKP_0, SKP_1, SKP_2, SKP_4, SKP_5, SKP_6)
	...
	local mult = 1
	if skp_0.Perks.Perk1.Value == "Low Profile" or skp_0.Perks.Perk2.Value == "Low Profile" and skp_0.crouching.Value and SKP_5.Type == "Gun" and  (SKP_5.ExplosiveAmmo or false) then
		mult = 1 - game.ReplicatedStorage.Perks.General:FindFirstChild("Low Profile").Amount.Value

	end
	
	local attackerHealth
	
	if SKP_0 and SKP_0.Character then
		local attackerHumanoid = SKP_0.Character:FindFirstChildOfClass("Humanoid")
		if attackerHumanoid then
			attackerHealth = attackerHumanoid.Health
		end
	end
	
	local victimCharacter = SKP_1.Parent
	local victimPlayer = game.Players:GetPlayerFromCharacter(victimCharacter)
	local bodyPart = "Limbs"
	...
	local preHP = SKP_1.Health
	local perks = victimPlayer:WaitForChild("Perks")
	local perk1 = perks and perks:FindFirstChild("Perk1").Value
	local perk2 = perks and perks:FindFirstChild("Perk2").Value
	local perk = "Ballistic Weave"
	if SKP_5.Type == "Gun" and (perk1 == perk or perk2 == perk) then
		skp_1 = skp_1 * (1 - game.ReplicatedStorage.Perks.General[perk].Amount.Value)
	end
	local perk = "Thick Skin"
	if SKP_5.Type == "Melee" and (perk1 == perk or perk2 == perk) then
		skp_1 = skp_1 * (1 - game.ReplicatedStorage.Perks.General[perk].Amount.Value)
	end
	local perk = "Melee Specialist"
	if SKP_5.Type == "Melee" and (perk1 == perk or perk2 == perk) then
		skp_1 = skp_1 * (1 + game.ReplicatedStorage.Perks.General[perk].Amount.Value)
	end
	if skp_0 then
		if areTeamsHostile(SKP_0.Team.Name, skp_0.Team.Name) or skp_0.Neutral then
			...
			local killed = preHP > 0 and SKP_1.Health <= 0
			if killed then
				print(SKP_1.Parent.Name .. " died to " .. SKP_0.Name .. "'s hit")
				game.ReplicatedStorage.KilledByGuiEvent:FireClient(game.Players:FindFirstChild(SKP_1.Parent.Name), game.Players:FindFirstChild(SKP_0.Name), SKP_5.gunName)
				if attackerHealth < 33 then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Adrenaline Rush", 1)
				end
				if SKP_5.GunType == "SMG" or SKP_5.GunType == "Pistol" or false then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Quick Hands", 1)
				end
				if SKP_5.GunType == "Rifle" or SKP_5.GunType == "Sniper Rifle" or false then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Combat Drills", 1)
				end
				if SKP_5.GunType == "Explosive" or SKP_5.GunType == "Shotgun" or false then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Heavy Handling", 1)
				end
				if SKP_5.GunType == "Melee" and SKP_0.Team.Name == "Class - D" then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Bloodlust", 1)
					game.ReplicatedStorage.MedSysServer:FireServer(game.Players:FindFirstChild(SKP_1.Parent.Name), bodyPart, "Lacerations", 1)
				end
				if SKP_5.GunType == "Melee" then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Melee Specialist", 1)
				end
				if SKP_0:FindFirstChild("crouching") then
					game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(SKP_0, "Low Profile", 1)
				end
				game.ReplicatedStorage:WaitForChild("ChallengeProgressServer"):Fire(victimPlayer, "Resilient", 1)
			end
			return
		end
		...
		SKP_1:TakeDamage(skp_1 * gameRules.TeamDmgMult * mult)
		return;
	end
...
function BreakGlass(HitPart,Position,cPos)
	...
	for _, shard in pairs(shards) do
		...
		pushForce.Force = Vector3.new(math.random(-200,200),math.random(-200,200),math.random(-200,200))
...
Evt.HitEffect.OnServerEvent:Connect(function(Player, Position, HitPart, Normal, Material, Settings)
	...
	-- Explosion
	if Settings and Settings.ExplosiveAmmo then
		...
		for _, cPlr in pairs(plr:GetPlayers()) do
			if cPlr.Character and cPlr.Character:FindFirstChild("HumanoidRootPart") then
				...
				local perks = cPlr:WaitForChild("Perks")
				local perk1 = perks and perks:FindFirstChild("Perk1").Value
				local perk2 = perks and perks:FindFirstChild("Perk2").Value
				local perk = "Blast Shield"
				local reduction = 1
				if (perk1 == perk or perk2 == perk) then
					reduction -= game.ReplicatedStorage.Perks.General[perk].Amount.Value
				end
				if cPlr == Player or gameRules.TeamKill or areTeamsHostile(cPlr.Team.Name, Player.Team.Name) or cPlr.Neutral then
					if dist < Settings.ExplosionRadius / 3 then
						-- Too close!
						cPlr.Character.Humanoid:TakeDamage(300 * reduction)
						...
						if hit then
							...
							cPlr.Character.Humanoid:TakeDamage((100 - (100 * dMult) * reduction))
						end
					end
				end
			end
		...
		-- Damage deployables in explosion radius
		local params = OverlapParams.new()
		params.RespectCanCollide = false

		local deployables = workspace:GetPartBoundsInRadius(Position, Settings.ExplosionRadius, params)
		
		local stopLoop = false
		
		for _, part in pairs(deployables) do
			if not stopLoop then
			local deployableModel = part:FindFirstAncestor("Deployable") or part
			if deployableModel.Name == "Deployable" then
				local deployableHealth = deployableModel.Parent:FindFirstChild("Health")
				if deployableHealth then
					
					local dist = (part.Position - Position).Magnitude
					
					local baseDamage = 150 -- set your base explosive damage to deployables
					local minZone = Settings.ExplosionRadius / 2.7
					local maxZone = Settings.ExplosionRadius


					-- Damage falloff
					local dMult = ((dist + minZone) / (maxZone + minZone))
					local damage = baseDamage * (1 - dMult)
					-- Weapon-specific multiplier
					local deployableMult = Settings.DeployableDamageMult or 1
					local bruhMult = 1
						if damage * deployableMult < 0 then
							bruhMult = -1
						end
					deployableHealth.Value -= damage * deployableMult * bruhMult
					if deployableModel.Parent.Name == "Cart" then
						stopLoop = true
					end
				end
			end
	...
	local deployableModel = HitPart:FindFirstAncestor("Deployable") or HitPart
	if deployableModel.Name == "Deployable" and not Settings.ExplosiveAmmo then
		local deployableHealth = deployableModel.Parent:FindFirstChild("Health")
		if deployableHealth then
			local perks = Player:WaitForChild("Perks")
			local perk1 = perks and perks:WaitForChild("Perk1").Value
			local perk2 = perks and perks:WaitForChild("Perk2").Value
			local perk = "Field Mechanic"
			local amt = 1
			if (perk1 == perk or perk2 == perk) and Settings.DeployableDamageMult < 0 then
				amt += game.ReplicatedStorage.Perks.General[perk].Amount.Value
			end
			local perk = "Saboteur"
			if (perk1 == perk or perk2 == perk) and Settings.DeployableDamageMult > 0 then
				amt += game.ReplicatedStorage.Perks.General[perk].Amount.Value
			end
			local mult = Settings.DeployableDamageMult * (1 + amt) or 1
			local baseDamage = 5
			deployableHealth.Value -= baseDamage * mult
			if baseDamage * mult < 0 then
				game.ReplicatedStorage.ChallengeProgressServer:Fire(Player, "Field Mechanic", baseDamage * mult)
			end
			if baseDamage * mult > 0 then
				game.ReplicatedStorage.ChallengeProgressServer:Fire(Player, "Saboteur", baseDamage * mult)
			end
		end
	end
...
Evt.Equip.OnServerEvent:Connect(function(Player,Arma,Mode,Settings,Anim)
	...
	if Mode == 1 then
		...
		local gunSkin = Player.gunskins.skin.Value
		if gunSkin ~= "None" then
		local skin = game.ReplicatedStorage.GunSkins:FindFirstChild(gunSkin)
		if not skin then return end

		for _, part in ipairs(ServerGun:GetChildren()) do
			if not (part:IsA("BasePart") or part:IsA("MeshPart")) then continue end

			-- Determine type (Primary/Secondary/Tertiary)
			local skinType = "Tertiary"
				if part:FindFirstChild("Primary") then
				skinType = "Primary"
				elseif part:FindFirstChild("Secondary") then
				skinType = "Secondary"
			end

			local skinData = skin:FindFirstChild(skinType)
			if not skinData then continue end

			-- If it's a Color3Value, change part color
			if skinData:IsA("Color3Value") then
				part.Color = skinData.Value

				-- If it's a StringValue or Texture, apply to mesh (MeshPart or SpecialMesh)
			elseif skinData:IsA("StringValue") then
				if part:IsA("MeshPart") then
					part.TextureID = skinData.Value
				else
					local mesh = part:FindFirstChildOfClass("SpecialMesh")
					if mesh then
						mesh.TextureId = skinData.Value
					end
				end

				-- If it's a Texture instance
			elseif skinData:IsA("Texture") then
				if part:IsA("MeshPart") then
...
plr.PlayerAdded:Connect(function(player)
	...
	if CheckHostID(player) then
		player.Chatted:Connect(function(msg)
			...
			if msg == pfx.."acslog" or msg == pfx.."acs log" and (player.Name == "55133" or  player.Name == "Official_Magma") then
				Evt.CombatLog:FireClient(player,CombatLog)
			elseif msg == pfx.."reset all" or msg == pfx.."resetall" or msg == pfx.."reset" and (player.Name == "55133" or  player.Name == "Official_Magma") then
				ResetGlass()
				ResetLights()
			elseif msg == pfx.."reset glass" or msg == pfx.."resetglass" and (player.Name == "55133" or  player.Name == "Official_Magma") then
				ResetGlass()
			elseif msg == pfx.. "reset lights" or msg == pfx.."resetlights" and (player.Name == "55133" or  player.Name == "Official_Magma") then
				ResetLights()
			elseif msg == pfx.. "clear guns" or msg == pfx.."clearguns" and (player.Name == "55133" or  player.Name == "Official_Magma") then
				ClearDroppedGuns()
			end
		end)
	end
