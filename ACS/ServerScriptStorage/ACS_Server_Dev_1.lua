...
Evt.HitEffect.OnServerEvent:Connect(function(Player, Position, HitPart, Normal, Material, Settings)
	...
	if deployableModel.Name == "Deployable" and not Settings.ExplosiveAmmo then
		local deployableHealth = deployableModel.Parent:FindFirstChild("Health")
		if deployableHealth then
			local mult = Settings.DeployableDamageMult or 1
			local baseDamage = 5
...
function loadAttachment(weapon,WeaponData)
	...
	--load sight Att
	if WeaponData.SightAtt ~= "" then

		local SightAtt = AttModels[WeaponData.SightAtt]:Clone()
		SightAtt.Parent = weapon
		SightAtt:SetPrimaryPartCFrame(weapon.AttNodes.Sight.CFrame)
	...
	--load Barrel Att
	if WeaponData.BarrelAtt ~= "" then

		local BarrelAtt = AttModels[WeaponData.BarrelAtt]:Clone()
		BarrelAtt.Parent = weapon
		BarrelAtt:SetPrimaryPartCFrame(weapon.AttNodes.Barrel.CFrame)
	...
	if WeaponData.UnderBarrelAtt ~= "" then

		local UnderBarrelAtt = AttModels[WeaponData.UnderBarrelAtt]:Clone()
		UnderBarrelAtt.Parent = weapon
		UnderBarrelAtt:SetPrimaryPartCFrame(weapon.AttNodes.UnderBarrel.CFrame)
	...
	if WeaponData.OtherAtt ~= "" then

		local OtherAtt = AttModels[WeaponData.OtherAtt]:Clone()
		OtherAtt.Parent = weapon
		OtherAtt:SetPrimaryPartCFrame(weapon.AttNodes.Other.CFrame)
